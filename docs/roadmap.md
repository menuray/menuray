...
- **✅ Session 8** (2026-04-26) — Merchant editorial polish. Pro+ tier removes the `menuray.com` wordmark from the brand-styled share PNG (`_QrShareCard.showWordmark` reads `currentTierProvider`). `MenuManagementScreen` time-slot radio now persists via `MenuRepository.updateMenu(timeSlot:)` — local-only state for the optimistic snap, network round-trip via the existing partial-PATCH. `MenuCard` 3-dot overflow opens a bottom sheet with "Duplicate menu" → new SECURITY DEFINER `duplicate_menu(p_source_menu_id)` RPC deep-clones categories + dishes + translations into a draft (status='draft', slug=NULL). Role-gated owner/manager via `user_store_role`; tier-gated via existing `assert_menu_count_under_cap`. Quiet S7 fix: `menus.available_locales` column declared + backfilled (translate-menu was reading/writing it without a schema entry). 5 new i18n keys; 4 new Flutter smoke tests (117 total); PgTAP billing_quotas extended with 2 duplicate_menu cases. ADR-025. Spec/plan `2026-04-26-merchant-polish*`.
- **✅ Session 9** (2026-04-26) — PDF table-tent generator. Wires the previously-hidden `publishedExportPdf` button on `PublishedScreen` to a new `PdfExportService` that builds an A4 portrait two-panel PDF (store name + vector QR + scan caption + URL + tier-aware menuray.com wordmark) via the pure-Dart `pdf` package. `share_plus` hands the file to the system sheet. ADR-026. Spec/plan `2026-04-26-pdf-table-tent*`.
...
Current test totals: 119 merchant Flutter · 25 customer Vitest + 8 Playwright e2e · 40 Deno tests (5 accept-invite + 4 create-checkout + 3 create-portal + 5 handle-stripe-webhook + 4 create-store + 5 log-dish-view + 5 export-statistics-csv + 5 translate-menu + 4 ai-optimize) · PgTAP analytics_aggregations + billing_quotas (extended with duplicate_menu cases in S8) + rls_auth_expansion. Branch: `main` only (no remote).
...
- [x] **S** Multi-menu (lunch / dinner / seasonal) — `MenuManagementScreen` time-slot radio persists via `MenuRepository.updateMenu(timeSlot:)` (Session 8). Customer-side display/filter still deferred to P2.
- [x] **S** Sold-out toggles persist — already shipped in S3 (`mark_dish_soldout` SECURITY DEFINER RPC + `MenuRepository.setDishSoldOut`); confirmed in S8 audit
- [x] **S** Menu duplication — overflow → bottom sheet "Duplicate menu" → `duplicate_menu` SECURITY DEFINER RPC deep-clones categories + dishes + translations into a draft (Session 8, ADR-025)
